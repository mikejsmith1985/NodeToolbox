...
human attention. A hygiene digest is delivered to Microsoft Teams so teams
receive a daily nudge toward better data quality.
...
- Q: How should issue owners be notified of unfixable violations? → A: A Jira comment is posted directly on the violating issue, and a team digest is delivered to Teams via the existing report webhook delivery pattern (same mechanism as Scope Change and Feature Change reports).
...
4. **Given** a user has fetched Jira issues in CHG wizard Step 1,
   ...
   **Then** a Short Description and Description are populated from the Jira issue
   content within a reasonable wait time, and the user may edit them before
   proceeding.
...
- Teams webhook for hygiene digest is not configured → the digest delivery is
  silently skipped; the scan still runs and applies fixes/comments normally.
...
- **FR-012**: After each scan, the system MUST deliver a hygiene digest to the
  team's configured Teams webhook via the existing report webhook delivery
  pattern. The digest payload MUST include: total issues scanned, violation
  count, fixes applied, actions required, and a trend indicator (change vs the
  previous scan). Digest delivery MUST be non-blocking: if no Teams webhook is
...
- **FR-014**: All hygiene monitor configuration (schedule time, weekday pattern,
  project keys, Teams webhook target, Jira field mappings for hygiene rules) MUST
  be managed exclusively in the Admin Hub. No configuration UI for this feature
  exists outside the Admin Hub.
...
- **SC-003**: CHG wizard users can populate Short Description and Description
  ...
  leaving the wizard or copying text manually.
