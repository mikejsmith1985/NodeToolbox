...
- [X] T001 Confirm work happens on `feature/020-agile-hub-home-ux` and the client gates run green pre-change:
...
- [X] T002 [P] RED — write `client/src/store/toolVisibilityStore.test.ts`: default-visible when unset,
...
- [X] T003 GREEN — implement `client/src/store/toolVisibilityStore.ts` (zustand; same key + value shape
...
- [X] T004 [P] [US1] RED — extend `client/src/views/Home/HomeView.test.tsx`: with admin locked the SNow card and
...
- [X] T005 [US1] GREEN — wire `client/src/views/Home/HomeView.tsx` to filter cards and recent chips through
      `resolveToolIsVisible` + per-card `gateKind` against `useAdminStore` (predicate per contracts/home-gating.md)
- [X] T006 [P] [US1] RED — extend `client/src/views/AdminHub/ToolVisibilitySection.test.tsx`: toggles read/write
      the shared store (not private helpers), `admin-hub` is not listed, retired card ids are not listed
- [X] T007 [US1] GREEN — rework `client/src/views/AdminHub/ToolVisibilitySection.tsx` to consume
      `toolVisibilityStore` (delete its private load/save/resolve helpers; the "persistence only" note dies)
- [X] T008 [US1] RED — extend `client/src/App.test.tsx`: `/snow-hub` while locked renders home; while
...
- [X] T009 [US1] GREEN — add the entry-gate wrapper in `client/src/App.tsx` for `/snow-hub` (admin unlock) and
...
- [X] T010 [P] [US2] RED — extend `client/src/views/Home/HomeView.test.tsx`: the three section headers render
...
- [X] T011 [US2] GREEN — rewrite the catalog in `client/src/views/Home/homeCardData.ts`: `SectionKey` →
...
- [X] T012 [US2] Reconcile dependent tests/fixtures that pin old sections or cards (`HomeView.test.tsx`,
...
- [X] T013 [P] [US3] RED — write `client/src/views/AgileHub/AgileHubView.test.tsx`: `?space=` resolution
...
- [X] T014 [US3] GREEN — implement `client/src/views/AgileHub/AgileHubView.tsx` +
...
- [X] T015 [US3] Add `agileHubLastSpace` to `client/src/store/settingsStore.ts` (with test in
      `client/src/store/settingsStore.test.ts` first — RED then GREEN in one task, persisted like the existing
      persisted UI fields)
- [X] T016 [P] [US3] RED — extend `client/src/App.test.tsx` with the redirect table from
...
- [X] T017 [US3] GREEN — implement the redirects in `client/src/App.tsx`: `RedirectToAgileHub` element (reads
...
- [X] T018 [US3] Verify-only sweep: confirm `client/src/views/SprintDashboard/`, `client/src/views/PoTool/`, and
...
- [X] T019 E2E — write and pass `test/e2e/agile-hub-home.spec.js` per the contract gates: (1) locked home has no
...
- [X] T020 [P] CHANGELOG.md entry under [Unreleased] (home reorganization, SNow gating, working visibility
      toggles, the Agile Hub merge + redirects)
- [X] T021 Capability-parity audit (SC-006): run the quickstart checklist against the built app with stubbed
      data — every enumerated tab of the three retired tools reachable in its space; record the checked list in
      the PR description
- [X] T022 Full gates: `cd client && npx vitest run && npx tsc -b && npx eslint src && npm run build`, then
