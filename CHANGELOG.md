...
- **Teams → Jira intake — per-row project routing via team mapping (feature 005)**: The submission `project` column carries a **team name** (e.g. "Cleanup Crew"); Intake settings now include a **team → project-key mapping** (e.g. "Cleanup Crew" → `ENCUC`) so each row is created in the right Jira project. Rows with a blank team use the **default project**; a team name with no mapping is flagged **Invalid** with a clear reason (never created in the wrong project). New pure `resolveProjectKey` helper + mapping editor in the settings panel.
- **Teams → Jira intake — queue now shows every captured field (feature 005)**: The intake queue previously displayed only status, summary, submitter, timestamp, and Jira key, which made imports look like they had dropped data. It now shows the **description and acceptance criteria** (as detail lines under the summary) plus dedicated **Type**, **Priority**, and **Team** columns — all values were always parsed and used at create time; they just weren't displayed.
