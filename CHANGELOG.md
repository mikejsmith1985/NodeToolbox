...
### Fixed
- **SNow relay doesn't auto-activate when bookmarklet is clicked on an already-open tab** — `crAutoPingBridge` (which polls the bridge status until the bookmarklet registers) was only started when the user clicked "Open SNow Tab" inside NodeToolbox. If the user already had a ServiceNow tab open and clicked the bookmarklet directly, `crAutoPingBridge` was never started, `CRG.relay.snowReady` was never set to `true`, and all requests fell through to the Basic Auth proxy (returning 401 on SSO-only instances). Four targeted fixes:
  1. `tbxProbeRelay` in server mode now handles the "bridge became active" case — when `/api/relay-bridge/status` returns `active: true` but `snowReady` is still `false`, it sets `snowReady = true`, updates the connection bar, and triggers `tbxOnSnowRelayConnect()`.
  2. New `tbxOnSnowRelayConnect()` — when the SNow relay transitions from inactive → active, if the RM view is open and `RM._lastLoaded` is null (the state after a proxy 401 or before first load), auto-triggers `rmLoadMyCHGs()`. The user no longer has to manually click "Load" after connecting.
  3. The 30-second keep-alive interval now probes all *configured* services (not only already-connected ones), so a manually-clicked bookmarklet auto-activates within 30 seconds without any further user action.
  4. On page load, a 2-second deferred probe checks whether any relay bridge is already registered — handles the case where the NodeToolbox server restarts while the bookmarklet tab is still open.
