...
- **Hygiene monitor scan engine (T025-T029)**: `runHygieneScan` now orchestrates a full scan: queries Jira for open issues, evaluates all 20 hygiene rules per issue, batches violations into Rovo classification prompts (50 violations/batch, non-blocking), applies FIXABLE field updates via the Jira proxy (rejected updates are re-classified as UNFIXABLE), posts UNFIXABLE comments with per-cycle dedup (one comment per issue+checkId pair per run), builds a trend digest, and delivers it to the team's Teams incoming webhook via `reportWebhookDelivery`. The Teams webhook host (`.webhook.office.com`) is added as a narrow, named exception in `webhookHostPolicy.js`. A `hygiene-digest` surface is registered in `reportSurfaceRegistry.js` and resolves the destination from `configuration.hygieneMonitor.teams`.
- **Hygiene monitor config persistence (T024)**: The `hygieneMonitor` section is now saved to and loaded from `toolbox-proxy.json`. Each team's `teamsWebhookSecret` is base64-obfuscated on disk (same pattern as other credentials) and never appears in API responses. Scan history is bounded to the 30 most recent entries (one month of daily scans). The config survives restarts and version upgrades.
