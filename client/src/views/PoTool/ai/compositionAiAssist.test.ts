...
describe('nine-section documentation (spec 029)', () => {
  it('the prompt instructs the nine sections, the markers, AC-in-both, and no AI attribution (US1/US2)', () => {
    const prompt = buildCompositionPrompt(buildDraft(), DEFINITION_OF_READY, WRITABLE_FIELDS);
    ['Description', 'Benefit Hypothesis', 'Acceptance Criteria', 'Assumptions', 'Dependencies',
      'In Scope', 'Out of Scope', 'Risks', 'Non-Functional Requirements (NFR)'].forEach((label) => {
      expect(prompt).toContain(label);
    });
    expect(prompt).toContain('⚠ REQUIRES BUSINESS VALIDATION');
    expect(prompt).toContain('⚠ REQUIRES TECHNICAL VALIDATION');
    expect(prompt).toContain('BOTH in the description\'s "Acceptance Criteria" section AND in the acceptanceCriteria field');
    expect(prompt.toLowerCase()).toContain('never state or imply that any of this was written, generated, or drafted by ai');
  });

  it('ingest normalizes a reply’s description to all nine sections, flagging missing ones (US1)', () => {
    const reply = buildReply({ summary: 'Login', description: 'Description: Users cannot log in.' });
    const proposal = parseCompositionIngest(reply, Object.keys(WRITABLE_FIELDS)).items[0];
    ['Description:', 'Benefit Hypothesis:', 'Assumptions:', 'Risks:', 'Non-Functional Requirements (NFR):'].forEach((heading) => {
      expect(proposal.description).toContain(heading);
    });
    expect(proposal.description).toContain('⚠ REQUIRES'); // a missing section is flagged
  });

  it('ingest strips AI self-attribution from the description (US2)', () => {
    const reply = buildReply({ summary: 'Login', description: 'Description: This was generated by AI. Users cannot log in.' });
    const proposal = parseCompositionIngest(reply, Object.keys(WRITABLE_FIELDS)).items[0];
    expect(proposal.description.toLowerCase()).not.toContain('generated by ai');
    expect(proposal.description).toContain('Users cannot log in.');
  });

  it('lists a select field’s allowed options so the AI picks a valid Initiative Type', () => {
    const prompt = buildCompositionPrompt(
      buildDraft(),
      DEFINITION_OF_READY,
      { customfield_10500: 'Initiative Type' },
      { customfield_10500: ['Business', 'Enabler', 'Compliance'] },
    );
    expect(prompt).toContain('"customfield_10500" (Initiative Type) — choose exactly one of: Business, Enabler, Compliance');
  });
});
