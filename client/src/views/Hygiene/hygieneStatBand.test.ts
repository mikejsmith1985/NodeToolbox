...
/** A delivery issue on a dated release, so every date flag it carries is one the fix can write. */
function finding(
  issueKey: string,
  flags: Array<[checkId: string, severity: 'error' | 'warn']>,
...
describe('buildHygieneStatBand', () => {
  it('leads with the four figures in the order they are acted on', () => {
    const band = buildHygieneStatBand([], 0, FIELD_CONFIG);
  ...
  it('counts issues, not flags', () => {
    ...
    const band = buildHygieneStatBand([finding('A-1', [['no-assignee', 'error'], ['no-ac', 'error']])], 10, FIELD_CONFIG);
  ...
  it('counts an issue with both an error and a warning only as broken', () => {
    ...
    const band = buildHygieneStatBand([finding('A-1', [['no-assignee', 'error'], ['missing-sp', 'warn']])], 10, FIELD_CONFIG);
  ...
  it('does not count an overdue flag as fixable — rewriting that date hides a true statement', () => {
    const band = buildHygieneStatBand([finding('A-1', [['due-date-overdue', 'warn']])], 10, FIELD_CONFIG);

    expect(band[2].count).toBe(0);
  });

  it('does not count an issue the policy already knows it cannot date', () => {
    // The band said "5 fixable, one click, no decisions" and the click wrote to none of them: three
    // had no dated release. The figure is the same selection the button then acts on (GH #384).
    const band = buildHygieneStatBand([
      finding('A-1', [['missing-due-date', 'warn']], { fixVersions: [] }),
      finding('A-2', [['missing-target-end', 'warn']], { fixVersions: [{ name: 'R-TBD', released: false }] }),
      finding('A-3', [['missing-due-date', 'warn']]),
    ], 10, FIELD_CONFIG);

    expect(band[2].count).toBe(1);
  });

  it('reports the rest of the scan as clean', () => {
    const band = buildHygieneStatBand([finding('A-1', [['missing-sp', 'warn']])], 31, FIELD_CONFIG);
  ...
  it('never reports a negative clean count', () => {
    ...
    const band = buildHygieneStatBand([finding('A-1', [['missing-sp', 'warn']]), finding('A-2', [['no-ac', 'warn']])], 1, FIELD_CONFIG);

    expect(band[3].count).toBe(0);
  });

  it('is all zeroes for a scan that found nothing to flag', () => {
    expect(buildHygieneStatBand([], 0, FIELD_CONFIG).every((stat) => stat.count === 0)).toBe(true);
  });
});
