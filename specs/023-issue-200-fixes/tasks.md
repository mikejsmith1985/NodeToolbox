...
- [X] T001 On `feature/023-issue-200-fixes` (worktrees per track), confirm gates run green pre-change:
...
- [X] T002 [US1] **Resolve server parity FIRST** (S1): determine whether `src/services/hygieneRules.js` runs an
      equivalent fix-version check; record the answer in a one-line comment. This gates whether US1 is client-only.
- [X] T003 [US1] RED — extend `client/src/views/Hygiene/checks/hygieneChecks.test.ts`: `checkMissingFixVersion` flags
      Story/Task/Defect/Feature/Epic with empty `fixVersions`, skips Sub-tasks and any issue that HAS a fix version
- [X] T004 [US1] GREEN — in `client/src/views/Hygiene/checks/hygieneChecks.ts` add
...
- [X] T007 [US2] RED — unit test in `client/src/views/Hygiene/utils/buildHygieneJqlUrl.test.ts` (+ clause tests beside
...
- [X] T008 [P] [US2] RED — component test for the tile affordance in `client/src/views/Hygiene/HygieneView.test.tsx`
...
- [X] T009 [US2] GREEN — co-locate a `jqlClause` with each predicate in
...
- [X] T010 [US2] GREEN — extend `client/src/views/Hygiene/utils/buildHygieneJqlUrl.ts`: `buildHygieneCheckJql(checkId,
...
- [X] T013 [P] [US3] RED — unit test `useQuickLookupStore` in
      `client/src/components/QuickIssueLookup/quickLookupStore.test.ts`: `open()` (null seed, nonce++), `open('ABC-1')`
      (seed set), `close()` (cleared)
- [X] T014 [P] [US3] RED — extend `client/src/components/QuickIssueLookup/QuickIssueLookup.test.tsx`: a `seedKey` prop
      drives `useIssueByKey(seedKey)` + prefills input; the gate opens when the store's `open` fires
- [X] T015 [P] [US3] RED — extend `client/src/components/IssueDetailPanel/index.test.tsx`: a linked-issue key renders as
      a focusable control that calls the store `open(key)` on activation [C1]
- [X] T016 [US3] GREEN — implement `client/src/components/QuickIssueLookup/quickLookupStore.ts`
      (`useQuickLookupStore { isOpen, seedKey, openNonce, open, close }`, zustand)
- [X] T017 [US3] GREEN — `QuickIssueLookupGate.tsx`: read open state from the store (F2 → `open()`), pass
...
- [X] T021 [P] [US4] RED — test `client/src/views/PoTool/PoTeamSelector.test.tsx` (create if absent): selecting an
...
- [X] T024 [US5] RED — component test for
...
- [X] T025 [US5] GREEN — restructure `BacklogRemediationPanel.tsx` so each item renders context (shared `IssueMeta`
...
- [X] T026 [US5] GREEN — hydrate `issuesByKey` (+`acceptanceCriteriaFieldIds`) on panel load (not only on Refresh),
...
- [X] T028 [P] [US6] RED — unit test `client/src/views/MyIssues/myIssuesRoleLens.test.ts`: role→emphasized-criteria per
...
- [X] T029 [P] [US6] RED — component test in `client/src/views/MyIssues/MyIssuesView.test.tsx` (create/extend):
      simulate-as banner ("Viewing as <name>" + Back to me), role-lens control changes emphasized sections, SM/PO team
      switch [C1]
- [X] T030 [US6] GREEN — implement pure `client/src/views/MyIssues/myIssuesRoleLens.ts` (role→criteria per the pinned
      set; roster-role→default lens; subject→assignee-JQL helper)
- [X] T031 [US6] GREEN — `client/src/views/MyIssues/hooks/useMyIssuesState.ts`: add `subject` (viewer|user|team), derive
      the "mine" assignee clause from it (swap `currentUser()`); read-only, no write path (FR-023)
- [X] T032 [US6] GREEN — `MyIssuesView.tsx`: "simulate as" Jira user-search (reuse `searchFeatureReviewUsers`) +
      "Viewing as <name>" banner with one-action **Back to me** (FR-019/020)
- [X] T033 [US6] GREEN — `MyIssuesView.tsx`: role-lens control defaulting from the subject's roster role, manually
