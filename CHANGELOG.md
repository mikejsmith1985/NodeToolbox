...
- **`miSnowSearchUser()` blocked in proxy mode** — The SNow identity search in My Issues Settings required `CRG.relay.snowReady` (relay bookmarklet active), blocking use in proxy mode even though `crRelayRequest` falls back to the server proxy for SNow. Changed the guard to `tbxSnowReady()` which returns `true` in both relay and proxy modes.
