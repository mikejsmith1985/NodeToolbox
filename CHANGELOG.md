...
### Added
- **Sprint–Release Workflow Orchestrator (feature 003 — Phases 1–2)**: Adds the scaffold and config foundation for the Jira Sprint–Release Workflow. New files: `src/services/sprintReleaseOrchestrator.js` (pure business logic for sub-status handoff detection, code-freeze date math, FixVersion date sync, defect intake, DoR violations), `src/services/sprintReleaseScheduler.js` (polling loop, in-memory ring buffers, run-now and status exports), and `src/routes/sprintRelease.js` (GET/POST /api/sprint-release/config with live boardId and project-key validation, /dor-violations, /run-now, /status). The `sprintRelease.teamProfiles` config block is persisted to `toolbox-proxy.json` via `buildDefaultConfig`, `saveConfigToDisk`, and `applyFileConfig` in `src/config/loader.js`. The scheduler starts on server boot alongside the other four schedulers. 52 unit tests added (config CRUD, orchestrator pure logic, scheduler poll-cycle wiring) — all GREEN.
