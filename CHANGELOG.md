...
- **Admin Hub → "⬆️ Sub-task → Story"**: bulk-promote sub-tasks into Stories that link back to their
  ...
  is the Move wizard, UI-only — so each promotion is create-Story → link-to-parent → optionally retire
  the original, three calls with no transaction between them. The panel therefore previews everything
  first, reports each step of each row independently, and keeps deleting the originals behind its own
  button that appears only for sub-tasks whose replacement Story was both created *and* linked.
  The link **direction** is resolved from the instance's own link-type catalogue rather than assumed:
  "contained within" is one half of a pair whose other half reads "contains", and putting the Story on
  the wrong side would make every promoted Story claim to contain its own parent. A new Story cannot be
  created directly into a chosen status, so where a single transition can reach the sub-task's status
  it is applied, and where none can the row says so instead of leaving the Story somewhere arbitrary.
...
- **Unplanned Work Mapping now lists the team's PI Review Features, not just one.** The Feature dropdowns discovered
  ...
  discovery the PI Review pull uses** — `issuetype = Feature` scoped by the PI and the **team's Product Owner(s)** from
  the imported roster (unscoped by project, since Features usually live in a portfolio project) — so both selectors
  offer exactly the Features that PI Review shows for that team and PI, including brand-new target Features. If the
  team's roster (with a flagged Product Owner) isn't imported yet, the panel says so rather than listing every
  Feature in the PI.

### Added
- **Type-to-filter on the Unplanned Work Mapping Feature pickers.** Each Feature dropdown now has a filter box above it
  — type any part of a key or summary to narrow a long PI's Feature list to the one you want.
...
- **F2 Quick Issue Lookup — find, view, and fix any issue from anywhere** (in progress): a global **F2**
  ...
  pressing F2 again while open re-focuses the search. All three stories are implemented and covered by a real-browser
  Playwright suite (`test/e2e/quick-issue-lookup.spec.js`, 12 scenarios: focused open + full context, empty-section
  omission, deep-link, honest states, normalization, in-place priority & assignee edits verified by the outgoing
  writes, a failed-write inline error, description read-only, recents persistence, swap-in-place + F2-while-open reset,
  and layout across light/dark themes, all three text sizes, and a narrow width). A field save now refreshes the issue
  in the background so the panel never flickers back to a spinner. Validation against a live Jira instance is the
  remaining follow-up.
...
- **Team Dashboard — Burndown could read a status as blank instead of failing loudly**: the burndown's per-day
  ...
  and the compiler now proves every path assigns one. No behaviour change today — this removes a way for a later
  change to go wrong quietly.
...
- **PO Tool — a home for Feature work (feature 017)**: a new **🧭 PO Tool** for the two jobs Product Owners
  ...
  - **Feature Splitter**: load a Feature by key, see its wording beside your own, and carve it into smaller
    Features that each deliver value — with always-on coaching on **how** to break it down (happy path first, by
    workflow step, by business rule, and five more, each with a worked example). Your split **saves as you go** and
    is still there days later. Before anything is written you see **every issue that will be created and every link
    that will be made**; only then does Jira change. A split **creates smaller Features and links them back to the
    original — it never closes, changes, or deletes the original**, and the new Features are the **same issue type**
...
- **PI Review — scheduled "Save to Confluence" (feature 015)**: a new **🗓️ PI Review Sync** panel in the Admin Hub
  ...
  a daily time, the **Product Owner** assignee, the **PI field id**, and the **PI Review page(s)** (URL + PI name) to
  refresh; a **Run now** button triggers an immediate refresh and each team shows its **last-run status**. A run
  appends the team's newly-matched PO+PI Features and refreshes the Jira-owned columns (priority, estimate,
  dependency, risks) exactly as the manual save does, while **preserving** every human-curated column, the Team
  Capacity snapshot, commitment boundary, grouping lines, and confidence votes — it never removes rows or empties the
  table, and a version conflict retries once then reports rather than clobbering a newer edit. A run **only writes
  when something actually changed** (a Feature appeared or a Jira-owned field moved), so an idle day never bumps the
  Confluence page version. Runs reuse the server's existing Jira/Confluence credentials (no new account), and every
  team's schedule is **disabled by default** — nothing runs until you opt it in. *(feature 015)*
- **Team Dashboard — Capacity: "Seed from Roster"**: the Capacity tab's **Team Composition** no longer has to be hand-built. A new **Seed from Roster** button (beside **+ Add Row**) fills the team makeup — one row per delivery role with its head count — directly from the active team's roster, so you only enter the capacity numbers (allocation %, PTO) by hand. Only **counting** roles are seeded: **Developer, Dev Lead, Internal Tester, External Tester, Systems Analyst**; **Scrum Master, Product Owner, Solution Architect, and Release Train Engineer** add no delivery capacity and are excluded (matching the Feature Canvas re-allocation planner's rule). The button is disabled until the roster has at least one member in a counting role, and — because a re-seed resets allocation %/PTO to defaults — it **asks before replacing** rows you already have. Each person is counted once (a delivery role wins over any coordination role they also hold).
...
- **PI Review — "Pull Features from Jira"**: a new edit-mode action on each PI Review page populates the table with **all of that team's Features for the sub-tab's PI**, so you no longer hand-enter them. It runs a single, direct Jira query — `issuetype = Feature AND assignee = <the team's Product Owner> AND cf[PI] = <the page's PI>` — where the **PI comes from the sub-tab** and the **Product Owner is read from the team roster** (the member flagged with the Product Owner capability; if a team lists more than one, all are matched). No label or assignee pickers to fill in, and **no project filter** (a team's Features often live in a portfolio/program project separate from its delivery board) — the PI plus the PO uniquely scope a team's Features. Newly pulled Features are appended (never duplicating a Feature already in the table) and immediately reconciled against Jira so priority, point estimate, dependencies, and risks fill in — then you review and **Save to Confluence** as usual. The pull is disabled with a hint until a Product Owner is flagged in the roster.
