...
### Added
- **Teams → Jira intake importer (feature 005, Phase 2 MVP)**: A new **Jira Intake** view lets a Toolbox user drag-and-drop the exported `Jira-Intake.xlsx`/CSV produced by the Teams "New Request" Power Automate flow, review the submissions in a newest-first queue, and create Jira issues from them. An **intake configuration** (target project + issue type + core-field→Jira-field mapping, reusing the Template Maker's pickers) is persisted in the shared Confluence content-property store, with an **auto-create-on-import** toggle. Each created issue sets the **reporter to the submitter** when their email resolves to a Jira user (new `searchUsers` wrapper using the Data Center `query`→`username` fallback), otherwise it falls back to the integration account (the `/jira-proxy` account). Dedup is **local by submission `id`** — re-importing the same or a superset file never double-creates. Submissions are parsed client-side with the bundled SheetJS parser and tolerate flat columns, dotted/nested keys, and extra columns. New `client/src/views/JiraIntake/` feature (lib + hooks + components) with co-located tests.
...
- **Teams → Jira intake — choice-drift guard + Home card (feature 005, Polish)**: A submission whose mapped choice value (e.g. a priority) is no longer among the Jira field's available options is now **flagged invalid** with a clear reason instead of attempting a malformed create (new name-based `findChoiceDrift`, since intake maps choices by name rather than option id). Added a **Jira Intake** card to the Home view (Agile & Delivery section).
