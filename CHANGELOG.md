...
### Added
- **SNow Hub — CHG Modify workflow for editing existing changes**: SNow Hub now includes a dedicated **CHG Modify** tab that lets users fetch existing ServiceNow Change Requests by key, modify all change details and planning fields, manage linked CTASKs (add, edit, remove), and save changes back to ServiceNow. The workflow mirrors the Create CHG 6-step wizard for familiarity, with streamlined field editing and inline CTASK operations replacing step-by-step guidance.
- **SNow Hub — CTASK template picker in both Create and Modify flows**: Both CHG creation and CHG modification workflows now include a unified **CTASK template picker** dropdown that lets users load pre-filled CTASK templates, and both flows allow saving the current CTASK as a new reusable template for future use. Templates are persisted locally in browser (max 20 per user) and include all CTASK fields: short description, description, assignment group, assignee, planned dates, and close notes.
- **SNow Hub — reusable CtaskEditForm component**: Extracted CTASK form logic into a reusable `CtaskEditForm` component that supports both create (CrgTab) and modify workflows. Component includes template loading via dropdown picker, inline save-as-template option, and full field validation via parent callbacks. CrgTab Create workflow now uses CtaskEditForm to render all CTASK form fields in the template editor, reducing code duplication and enabling future template management features.
- **SNow Hub — CHG Modify tab**: SNow Hub now includes a dedicated **CHG Modify** tab for editing and managing existing changes in ServiceNow, complementing the existing CHG creation workflow.
