...
### Fixed
- **Reports Hub — Feature Report now falls back to ART feature-scope projects, and the workspace now opens on the first tab**: when team project queries return no feature records, Reports Hub now retries the Feature Report against the ART-wide `featureProjectKeys` configured in ART settings, which matches the shared feature-scope behavior used elsewhere in the ART tooling and prevents an empty Feature Report for ARTs whose features live outside the team projects. Reports Hub also now launches on **Defect Dashboard**, which is the first visible tab, instead of defaulting internally to **Feature Report**.
