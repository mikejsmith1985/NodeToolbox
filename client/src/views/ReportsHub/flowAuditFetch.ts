// flowAuditFetch.ts — Pages a person's issues out of Jira, bounded by two ceilings.
//
...
// and a whole roster over all history is one click away. So there are two ceilings — one per person,
...
/** Most issues analysed for any one person before their figures are declared partial. */
export const PER_PERSON_ISSUE_CEILING = 500;

/**
 * Most issues analysed across a whole roster run. A per-person ceiling alone does not bound a roster:
 * fifteen people each just under their own limit is still fifteen times the work.
 */
export const RUN_ISSUE_BUDGET = 5_000;

/** Which limit stopped the analysis, when one did. */
export type FlowFetchCeiling = 'per-person' | 'run-budget';

/** What a person's paged fetch produced, and whether it was complete. */
export interface PersonFetchOutcome<TIssue> {
  issues: TIssue[];
...
export interface PersonFetchOptions {
  /** How much of the whole-run budget is still available when this person starts. */
...
/**
 * Fetches every issue for one person, page by page, stopping at whichever limit binds first.
 *
 ...
 */
export async function fetchAllPersonIssues<TIssue>(
  fetchPage: (startAt: number) => Promise<TIssue[]>,
  options: PersonFetchOptions,
): Promise<PersonFetchOutcome<TIssue>> {
  const allowance = Math.min(PER_PERSON_ISSUE_CEILING, Math.max(0, options.remainingRunBudget));
  ...
  const bindingCeiling: FlowFetchCeiling =
    options.remainingRunBudget < PER_PERSON_ISSUE_CEILING ? 'run-budget' : 'per-person';
