...
function buildSelectedNode(): CanvasNode {
  return {
    ...
    childStories: [], dependencies: [], businessValue: null, description: null, acceptanceCriteria: null, parkReason: null, attachments: [], effectivePoints: 0,
  };
}
...
describe('CoachPanel', () => {
  it('shows the current stage and jumps to another stage on click', () => {
    ...
    render(<CoachPanel controller={controller} selectedNode={null} wip={NO_WIP} onAddContainer={vi.fn()} onOpenCommit={vi.fn()} isAiUnlocked={false} onOpenAi={vi.fn()} />);
  ...
  it('hides the AI action when locked and shows it when unlocked', () => {
    ...
    const { rerender } = render(<CoachPanel controller={locked} selectedNode={null} wip={NO_WIP} onAddContainer={vi.fn()} onOpenCommit={vi.fn()} isAiUnlocked={false} onOpenAi={vi.fn()} />);
    expect(screen.queryByRole('button', { name: /AI suggestions/ })).not.toBeInTheDocument();

    const onOpenAi = vi.fn();
    rerender(<CoachPanel controller={locked} selectedNode={null} wip={NO_WIP} onAddContainer={vi.fn()} onOpenCommit={vi.fn()} isAiUnlocked onOpenAi={onOpenAi} />);
