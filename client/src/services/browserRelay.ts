...
const SNOW_RELAY_BODY = [
  ...
  // A base64 body is a FILE (a zip of test evidence) crossing the JSON envelope: it is decoded back to
  // bytes and sent with the caller's Content-Type, never re-serialized as JSON. Everything else is
  // the JSON path exactly as before.
  'function decodeBase64Body(encodedText){var binaryText=atob(encodedText);var decodedBytes=new Uint8Array(binaryText.length);for(var byteIndex=0;byteIndex<binaryText.length;byteIndex++){decodedBytes[byteIndex]=binaryText.charCodeAt(byteIndex);}return decodedBytes;}',
  'async function executeRelayRequest(relayRequest){try{var isBinaryBody=relayRequest.bodyEncoding==="base64";var requestHeaders={"Content-Type":isBinaryBody?(relayRequest.contentType||"application/octet-stream"):"application/json","Accept":"application/json","X-Requested-With":"XMLHttpRequest"};var requestToken=resolveUserToken();if(requestToken)requestHeaders["X-UserToken"]=requestToken;if(relayRequest.authHeader)requestHeaders["Authorization"]=relayRequest.authHeader;var requestController=new AbortController();var timeoutId=setTimeout(function(){requestController.abort();},relayRequest.timeoutMs||25000);var requestOptions={method:relayRequest.method||"GET",credentials:relayRequest.authHeader?"omit":"include",headers:requestHeaders,signal:requestController.signal};if(relayRequest.body!=null)requestOptions.body=isBinaryBody?decodeBase64Body(relayRequest.body):JSON.stringify(relayRequest.body);var targetUrl=location.origin+relayRequest.path;var serviceNowResponse=await fetch(targetUrl,requestOptions);clearTimeout(timeoutId);var responseText=await serviceNowResponse.text();await postRelayResult({id:relayRequest.id,sys:sys,ok:serviceNowResponse.ok,status:serviceNowResponse.status,data:responseText,error:null});}catch(requestError){await postRelayResult({id:relayRequest.id,sys:sys,ok:false,status:0,data:null,error:requestError.message});}}',
