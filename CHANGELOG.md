...
### Changed
- **Release Notes report — copy to clipboard, neutral heading**: The rendered release-notes report now has a **"📋 Copy Release Notes"** action that copies the section to the clipboard as an image (paste straight into email or chat) instead of downloading a PNG file; a brief "Copied to clipboard" confirmation appears after a successful copy (and is excluded from the captured image). The report heading no longer says "Rovo Release Notes Draft" — it is now built from the dashboard **team name** and the release **fix version**, e.g. *"Transformers 06/23/2026 Release Notes"* (or *"06/23/2026 Release Notes"* when no team is set), so the report carries no mention of how the notes were drafted. New `buildReleaseNotesHeading` helper in `releaseRovoNotes.ts`; `ReleasesTab` now receives the active team name. 5 tests added/updated; copy-to-clipboard path verified end-to-end in a real browser.
