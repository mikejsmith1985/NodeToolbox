...
export function buildRulePrompt(): string {
  ...
  return [
    ...
    '',
    'Prefer one of these known event types when it fits:',
    eventList,
    '',
    'If none of the above fits (e.g. a review was APPROVED, or a PR was CLOSED without merging) you MAY coin a',
    'NEW event type: a short lowercase snake_case slug such as pr_approved or pr_closed (letters, digits, and',
    'underscores only). A new event type posts a plain Jira comment and never changes an issue\'s status on its own.',
    '',
...
export function buildBulkRulePrompt(samples: EmailSample[]): BulkRulePromptResult {
  ...
  const prompt = [
    ...
    '',
    'Prefer one of these known event types when it fits a rule:',
    eventList,
    '',
    'If an email fits NONE of the above (e.g. a review was APPROVED, or a PR was CLOSED without merging) you MAY',
    'coin a NEW event type for it: a short lowercase snake_case slug such as pr_approved or pr_closed (letters,',
    'digits, and underscores only). A new event type posts a plain Jira comment and never changes a status on its own.',
    '',
