...
- [ ] T018 [P] [US2] Write a failing test in `client/src/views/ReportsHub/ReportAiPanel.test.tsx`: when `onRunAuto`
...
- [ ] T019 [US2] Extend `client/src/views/ReportsHub/ReportAiPanel.tsx` with **optional** `onRunAuto?` and
...
- [ ] T020 [P] [US2] Write failing component tests in `client/src/views/ArtView/ai/PiReviewAiPanel.test.tsx`: hidden
  ...
  (FR-012)
- [ ] T021 [US2] Create `client/src/views/ArtView/ai/PiReviewAiPanel.tsx` using the extended `ReportAiPanel` as the
...
- [ ] T022 [US2] Write a failing test in `client/src/views/ArtView/PiReviewTab.test.tsx`: the AI Assistance
...
- [ ] T023 [US2] Mount the panel in `client/src/views/ArtView/PiReviewTab.tsx` behind `canEditContent` (line 766) so
...
- [ ] T024 [US2] Prove US2: `cd client; npx vitest run src/views/ArtView src/views/ReportsHub` green. Manually walk
...
- [ ] T025 [P] [US3] Write failing unit tests in `client/src/views/ArtView/ai/piReviewAiApply.test.ts` for
...
- [ ] T026 [US3] Write failing unit tests in `client/src/views/ArtView/ai/piReviewAiApply.test.ts` (same file as
...
- [ ] T027 [US3] Write failing unit tests in `client/src/views/ArtView/ai/piReviewAiApply.test.ts` (same file as
...
- [ ] T028 [US3] In `client/src/views/ArtView/piReviewJira.ts`: **export** the existing module-private
...
- [ ] T029 [US3] Create `client/src/views/ArtView/ai/piReviewAiApply.ts` — `applyPiReviewSuggestion(row, suggestion)`
...
- [ ] T030 [P] [US3] Write failing component tests in
...
- [ ] T031 [US3] Create `client/src/views/ArtView/ai/PiReviewSuggestionTable.tsx` — the feature's one genuinely new
  ...
  Make T030 green
- [ ] T032 [US3] Write a failing test in `client/src/views/ArtView/ai/PiReviewAiPanel.test.tsx` for the accept
...
- [ ] T033 [US3] Wire the table into `PiReviewAiPanel.tsx`: Accept calls `applyPiReviewSuggestion` and marks the page
...
- [ ] T034 [US3] Write a failing test in `client/src/views/ArtView/ai/PiReviewAiPanel.test.tsx`, then implement in
...
- [ ] T035 [US3] Prove US3: `cd client; npx vitest run src/views/ArtView` green. Manually walk **quickstart
...
- [ ] T036 [P] [US4] Write failing component tests in `client/src/views/ArtView/ai/PiReviewSizingCard.test.tsx`:
...
- [ ] T037 [US4] Create `client/src/views/ArtView/ai/PiReviewSizingCard.tsx` rendering the scale from the shared
  constant — never a second hardcoded copy (FR-033, FR-034). Make T036 green
- [ ] T038 [US4] Mount the card in `client/src/views/ArtView/PiReviewTab.tsx` **outside** the unlock gate and
...
- [ ] T039 Update `CHANGELOG.md` under `## [Unreleased]`: **Fixed** — Ctrl+Alt+Z raised up to four stacked
...
- [ ] T040 Verify the **server is untouched**: `npx jest` and `npm run build:pi-review-engine && npm run test:dom`
  (13/13) both green — proving the `appendUniqueNoteLine` export (T028) did not disturb the engine feature 015 runs
  server-side (FR-037)
- [ ] T041 Verify **reconciliation is unchanged** (FR-038): the `Jira updated N fields on load` banner behaves as
  before, and `description`/`acceptanceCriteria` never appear in it — they are prompt inputs only (CW-5)
- [ ] T042 Full sweep: `cd client; npx vitest run && npx eslint src && npx vite build`. Confirm no new runtime
  dependency entered `package.json`
- [ ] T043 Walk the **Regression checks** section of `specs/016-pi-review-ai-assist/quickstart.md` — manual
