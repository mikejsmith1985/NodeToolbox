...
### Fixed
- **SNow Diagnostic: 401 errors misreported as missing data** — All three live tests in the SNow diagnostic report now correctly distinguish HTTP 401 (auth failure) from legitimate "no data" outcomes:
  - **Test 1**: When the connectivity ping returns 401, the report now says "credentials rejected (HTTP 401)" and directs the user to re-click the bookmarklet, noting that Okta/SSO instances reject Basic Auth. Previously showed the generic "check proxy URL and credentials" hint.
  - **Test 2**: When the identity lookup returns 401, the report now shows "HTTP 401 — auth failure (not a missing identity record)" instead of misleadingly saying "No SNow user found" — which sent users hunting for an identity that was actually fine.
  - **Test 3**: When the My Changes query returns 401, the "Possible causes" section now leads with the auth failure explanation and relay reconnection steps instead of the identity/filter hints that are irrelevant until credentials are resolved.

— When the ServiceNow proxy credentials were invalid or expired, `rmLoadMyCHGs` called `.then(r => r.json())` without checking `r.ok`. A 401 response from SNow still returns a JSON error body (no `result` key), so the code silently treated it as "no changes found" and displayed "No active changes assigned to Smith, Michael." Now checks `r.ok` first and throws a descriptive error that routes through the existing 401 catch handler, surfacing "SNow credentials invalid or expired — update them in Toolbox Settings → ServiceNow Connection."
