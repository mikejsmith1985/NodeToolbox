...
### Added
- **Jira Template Maker — foundational layer (feature 004, in progress)**: Groundwork for a guided tool that builds reusable, globally-shared Jira issue templates (Project → Issue Type → Field pickers, type-aware values, one-click create). This increment lands the fully-tested, I/O-free core only — no UI is wired yet. New: createmeta + create-issue types in `client/src/types/jira.ts`; `getCreateMeta`/`createIssue`/`getMyself` wrappers in `client/src/services/jiraApi.ts`; a globally-shared template store (`loadJiraTemplates`/`saveJiraTemplates` + 3-way merge under the new `nodetoolbox-jira-templates` Confluence content-property key) in `client/src/services/confluenceApi.ts`; and pure helpers under `client/src/views/JiraTemplateMaker/lib/` — `fieldModel` (createmeta→supported-field mapping), `wikiMarkup` (editor→Jira wiki-markup for Server/DC), `labels` (case-sensitive dedupe), and `buildCreatePayload`. 41 unit tests added, all GREEN; `tsc` clean; no regressions (527 client tests across new + touched suites). Remaining: the React wizard/components, launch flow, and live-Jira validation (see `specs/004-jira-template-maker/tasks.md`).
