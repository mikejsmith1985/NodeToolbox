...
### Fixed
- **Dev Panel — GitHub Debug tab for real-time GitHub connectivity diagnostics**: Added a new **GitHub Debug** tab to the Dev Panel with a button to fetch and display GitHub API authentication details and connectivity probe results. Shows the exact authorization header format being sent, masked PAT, base URL, and probe endpoint response (status code, response time, success indicator). Includes detailed help text explaining common GitHub connectivity issues (HTTP 401, 403, network errors) so users can diagnose connectivity problems without browser devtools. All debug info is masked to avoid exposing credentials in screenshots/logs.
...
- **SNow Hub — hidden Rovo CHG prompt now includes richer Jira context**: The generated prompt now includes each selected issue’s Jira description and acceptance criteria (`customfield_10200`) so AI-assisted CHG drafting has better deployment context than key/summary alone.
