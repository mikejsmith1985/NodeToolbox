...
- **Relay activation no longer navigates away from the CRG wizard**: When the user clicks "Open SNow Relay" the current route (`/snow-hub`, etc.) is stored in `localStorage`. After the SNow bookmarklet reloads the NodeToolbox window to the root URL, `App.tsx` automatically redirects back to the saved route so the user lands exactly where they were — CRG form data intact, dropdowns loading immediately.
