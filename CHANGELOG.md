...
### Fixed
- **SNow Hub — Release Management now uses the relay-backed change fetch path**: the Release Management tab was forcing ServiceNow change lookups through the direct server proxy, which could return 401 errors even when the browser relay was connected and working elsewhere in SNow Hub. Release Management now uses the same relay-backed `snowFetch` path as the CHG tools for both single-change lookups and **My Active Changes**, so browser-authenticated ServiceNow sessions load release data consistently.
