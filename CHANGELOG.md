...
### Fixed
- **CHG Generator now sets the Planned start/end dates on created changes**: New change requests created from the CHG Generator left **Planned start date** and **Planned end date** empty in ServiceNow. The create payload (`buildChangeRequestPayload` in `useCrgState.ts`) wrote the window to `planned_start_date`/`planned_end_date`, but on the `change_request` table those form fields are actually `start_date`/`end_date` (confirmed via the field inspector — `planned_*` only exist on `change_task`). ServiceNow's Table API silently drops unknown fields on insert, so both dates were discarded. The payload now writes `start_date`/`end_date`, matching the read path (`useReleaseManagement.ts`) and the modify-PATCH path (`api.js`). The three change-payload test assertions were updated accordingly (CTASK assertions, which legitimately use `planned_*`, are unchanged); 76 SnowHub tests GREEN.
