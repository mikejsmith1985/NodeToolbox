...
### Added
- **SNow Hub — browser console diagnostics for API debugging**: All SNow Hub API helper functions (Fetch My Changes, Fetch Change, Save Change) now log detailed diagnostics to the browser console, including endpoint URLs, HTTP status codes, response counts, and error details. Emoji prefixes (📋 📋 fetch, 💾 save, ✅ success, ❌ errors) make it easy to scan console output. When API calls fail, users can open DevTools (F12 → Console) to see the exact error, response body excerpt, and HTTP status for faster troubleshooting of relay or connectivity issues.
- **SNow Hub — My Open Changes feature**: New `/api/snow-relay/my-changes` endpoint fetches user's open ServiceNow changes (Open, Pending, or In Progress state). ModifyChgTab now includes a "Load My Open Changes" button with dropdown selector, eliminating manual change key entry. Endpoint supports optional state filter query parameter and gracefully falls back when user lookup fails.
