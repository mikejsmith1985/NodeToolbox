...
### Added
- **Rovo insight block on standup briefings (foundational + first surface)**: First increment of the Rovo-expansion feature (spec `002-rovo-expansion-hygiene-monitor`). Adds a shared, non-blocking server-side Rovo helper (`requestRovoText` in `src/services/rovoEnrichment.js`) that dispatches a prompt and polls for the result via the existing exchange, returning `null` (never throwing) on disable/timeout/error so report delivery is never blocked. The standup briefing scheduler now uses it to prepend a Rovo-authored insight block above the data tables when Rovo is enabled; when Rovo is unavailable the briefing publishes unchanged with a logged skip. Scope/feature-change schedulers, the CHG wizard actions, and the proactive hygiene monitor are subsequent increments of the same feature.
